...
@export var LineScn : PackedScene
@export var loopCount := 5_000_000: set = setLoopCount
@export var repeat := 3
...
func addMeasure(measureNode : MeasureBase, nodeName : String) -> void:
	...
	var line = LineScn.instantiate()
	$"VBoxContainer".add_child(line)
	line.get_node("Run").text = measureNode.name
	line.get_node("Run").connect("pressed", Callable(self, "run").bind(measureNode, line, repeat))
