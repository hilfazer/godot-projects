...
var _isSerializableFn : Callable
...
func setDefaultIsNodeSerializable():
	_isSerializableFn = Callable( self, "_isSerializable" )
...
func saveToFile( filepath : String ) -> int:
	var baseDirectory : String = filepath.get_base_dir()
...
func loadFromFile( filepath : String ) -> int:
	...
	if not FileAccess.file_exists( pathToLoad ):
